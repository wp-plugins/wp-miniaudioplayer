	...
	jQuery.mbMiniPlayer = {
		...
		buildPlayer: function (options) {
			...
			this.each(function (idx) {
				...
				player.fileUrl = encodeURI(player.opt.mp3 || player.opt.m4a);
				...
				if (player.opt.downloadable) {
					...
					var isSameDomain = (player.fileUrl.indexOf(host) >= 0) || player.fileUrl.indexOf("http") < 0;
					...
					if (!player.opt.downloadPage) {
						...
						if(isSameDomain && typeof a.download != "undefined"){
							...
							download = jQuery("<a/>")
									...
									.attr({href: player.fileUrl, download: player.fileName + "." + fileExtension})
									.css({display: "inline-block", cursor: "pointer"})
									.html("d")
									.attr("title", "download: " + player.fileName)
									.on("mouseover", function(){
										jQuery(this).attr("title", "download: " + player.fileName);
									});
						}
						...
						//if not open a new page with the audio file
							...
							download = jQuery("<span/>")
									...
									.on(jQuery.mbMiniPlayer.eventEnd, function () {
										window.open(player.fileUrl, "map_download");
									})
									.attr("title", "open: " + player.fileName);
						...
						download = jQuery("<span/>")
								...
								.on(jQuery.mbMiniPlayer.eventEnd, function () {
									...
									location.href = player.opt.downloadPage + "?filename=" + player.fileName + "." + fileExtension + "&fileurl=" + player.fileUrl;
								}).on("mouseover", function(){
									jQuery(this).attr("title", "download: " + player.fileName);
								})
								.attr("title", "download: " + player.fileName);
